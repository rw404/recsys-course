...
await page.getByText('Last completed run · Draft not applied', { exact: true }).waitFor()
await page.waitForTimeout(180)
...
await page.getByText('Pipeline is running · Results stay frozen', { exact: true }).waitFor()
...
const sourceNode = page.locator('.react-flow__node').filter({ hasText: 'ML-100K' })
