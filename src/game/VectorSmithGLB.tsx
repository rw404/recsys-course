...
const CLIP_FILES: [string, string][] = [
  ...
  ['think', `${DIR}/think.glb`], // 317 shrug (weighing)
]
const CLIP_URLS = CLIP_FILES.map(([, url]) => url)

// per-page gesture sequences (index = slide) — calm, natural talking poses.
const PAGE_GESTURES: string[][] = [
  ...
  ['think', 'talkRight', 'talkOpen'], // 3 negatives (weighing)
  ['talkPassion', 'talkOpen', 'talkRight'], // 4 in-batch finale
]
const CYCLE_SEC = 5.2
