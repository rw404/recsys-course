...
await p.goto('http://127.0.0.1:4173/?capture=1&nofx=1&lsnap=1',{waitUntil:'load',timeout:60000})
await p.waitForSelector('canvas'); await p.waitForTimeout(2800)
...
await p.screenshot({path:'renders/final_p0.png'}); console.log('final_p0')
const next=p.getByRole('button',{name:/Next/})
await next.click(); await p.waitForTimeout(450)
await p.screenshot({path:'renders/final_p1.png'}); console.log('final_p1 (nod gesture)')
await next.click(); await p.waitForTimeout(450)
await next.click(); await p.waitForTimeout(450)
await p.screenshot({path:'renders/final_p3.png'}); console.log('final_p3 (lean gesture)')
await b.close()
